...
def bfs(root, root2=[]):
    ...
    while queue:
        ...
        last_valve = v[0][-1]
        if (last_valve in valves_with_flow_rate_set) and not (last_valve in explored):
...
def compute_score_pairs(_paths):
    ...
    total_score_obtainable = _score
    for valve in (valves_with_flow_rate_set - valve_visited):
        total_score_obtainable += (26 - (max_steps + 1)) * valve_flow_rate[valve]

    if total_score_obtainable <= max_score:
        return -1
    return _score
...
with open('test_input.txt', 'r') as f:
    for line in f:
        ...
        if _flow_rate != 0:
            valves_with_flow_rate_set.add(_valve)
...
goals = truncate_goals(goals, NUM)
...
while not found:
    ...
    for path in path_valves:
        if (len(path[0]) + len(path[1])) == valves_with_flow_rate:
            ...
            new_goals1 = truncate_goal_pairs(new_goals1, path[1], NUM)
            new_goals2 = truncate_goal_pairs(new_goals2, path[0], NUM)
            ...
            # for ng1 in new_goals1:
            #     if ng1[1] < 26:
            #         tst = (path[0] + [ng1], path[1] + [ng1])
            #         new_goals.append((path[0] + [ng1], path[1] + [ng1]))
            #
            # for ng2 in new_goals2:
            #     if ng2[1] < 26:
            #         new_goals.append((path[0] + [ng2], path[1] + [ng2]))
...
# score: 2699
# score: 2963
print(f"The best score is: {max_score}")
print(f"Paths are: {max_solution}")
